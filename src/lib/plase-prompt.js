...
## LE SEI DIMENSIONI
- V (Valenza) [-1,+1]: disagio(-1), neutralità(0), benessere(+1). Attenzione alla V dissimulata (Pennebaker: ufemismi, minimizzzazioni).
...
Livello 1 (ALERT): Se V<-0.70 AMDA-0.60 -> alert supervisore.
Livello 2 (ADATTIVO):
...
Sicuro: diretto, articolato. Evitante: breve, fattuale — NON foryare. Preoccupato: lungo, urgente — contenere. Disorganizzato: frammentato — stabilità massima + supervisore.

## TRE SE DI DAMASIO
Proto-sé = corporeo = somatizzazione. Sé nucleare = finestra aperta. Sé autobiografico = narrative_coherence.

## KB SEED
vergogna: V=-0.78 A=+0.70 R=+0.88 Phi_tip=1.8
"non ci riesco piÙ": V=-0.60 A=-0.20 R=+0.55 Phi_tip=1.5
"sento un peso": V=-0.70 A=-0.75 R=+0.72 Phi_tip=1.3 [FIRMA FREEZE]
"non cambierà mai": V=-0.75 A=-0.40 R=+0.20 Phi_tip=1.2 [SCRIPT BEENE]
...
### ESERCIZI PER WINDOW STATECLOSED_BELOW (freeze dorsale):
...
UNSTABLE:
- Diario breve: "Scrivi 3 righe su come ti senti adesso. Non di picù."
...
export function parsePlaseOutput(text) {
  ...
  return {
    ...
    plase_emb: getFloat(/EMBODIMENT LEVELU�?\*?:\s*([\d.]+)/),
